...
import { Shield, Sparkles, Eye, Sun, Moon, Pencil, BrainCircuit, BookOpenCheck, Star, TramFront, Backpack, HeartCrack, Crown, LandPlot, ShieldCheck, UserCog } from "lucide-react";
...
const THIEF_RANKS: Record<number, string> = {
    1: 'Thief',
    2: 'Cutpurse',
    3: 'Footpad',
    5: 'Burglar',
...
const DRUID_DEGREES: Record<number, string> = {
  1: 'Novice',
  2: 'Neophyte',
  3: 'Outer Grove Initiate',
  5: 'Inner Grove Initiate',
  7: 'Limbo Ascendant',
  10: 'Adeptus',
  12: 'Naturis Minor',
  15: 'Naturis Prime',
...
export default function CharacterSheetPage() {
  ...
  const handleInventoryChange = (newInventory: Item[]) => {
    ...
    if (rankList) {
      ...
      const degreeLevel = Object.keys(DRUID_DEGREES)
        .map(Number)
        .sort((a, b) => b - a)
        .find(level => character.level >= level);

      let newDegree: string | undefined;
      if (degreeLevel) {
        newDegree = DRUID_DEGREES[degreeLevel];
      }

      if (newDegree && newDegree !== character.degree) {
        updateDoc(characterRef, { degree: newDegree });
        toast({
          title: "Degree Advanced!",
          description: `You have advanced to the degree of ${newDegree}.`,
        });
      }
    }
  ...
  const { leadershipScore, leadershipPenaltyText } = useMemo(() => {
    ...
    if (character.background === 'noble') {
        ...
        if (['prince', 'princess'].includes(character.background.toLowerCase())) score += 1;
        if (['baron', 'baroness', 'sheikh'].includes(character.background.toLowerCase())) score += 2;
        if (['duke', 'duchess', 'sultan', 'sultaness'].includes(character.background.toLowerCase())) score += 3;
    }
  ...
  const isRankedClass = ['Soldier', 'Engineer', 'Pilot', 'Phase Craft Technician', 'Thief', 'Healer', 'Monk'].includes(character.class);
  ...
  const getNobleTitleAndAssets = () => {
    if (character.level === 20 && character.background === 'noble') {
        const isFemale = character.gender === 'female';
        let title: string;
        if (character.faction?.toLowerCase() === 'sudaran') {
            title = isFemale ? 'Queen' : 'King';
        } else if (character.faction?.toLowerCase() === 'mizarian') {
            title = isFemale ? 'Sultaness' : 'Sultan';
        } else {
            return { title: `Ruler of ${character.faction}`, assets: null }; // Homebrew
        }
        ...
        return {
            ...
            assets: {
                land: '500,000 acres',
                retinue: ['War', 'Commerce', 'Health', 'Agriculture', 'Transportation', 'Exploration'].map(role => ({ role, name: `Advisor of ${role}`})),
                crownGuard: 120,
                kingsCompany: {
                    total: 160,
                    commandingOfficer: '1 Commanding Officer',
                    executiveOfficer: '1 Executive Officer',
                    engineerPlatoon: {
                        leader: '1 Engineer Platoon Leader (1st or 2nd Lt)',
                        warrantOfficers: '3 Warrant Officers',
                        enlisted: '1 Warrant Officer 4',
                    }
                }
            }
        };
    }
    return { title: null, assets: null };
  }

  const { title: nobleTitle, assets: nobleAssets } = getNobleTitleAndAssets();
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
              ...
              <div className="relative h-48 w-48 rounded-lg overflow-hidden border-2 border-primary/20 shadow-lg shadow-primary/10">
               ...
               <CardHeader className="p-4">
                  <CardTitle className="font-headline text-5xl">{character.name}</CardTitle>
                   {nobleTitle && (
                    <CardDescription className="text-2xl text-yellow-400 font-bold">
                      {nobleTitle}
                    </CardDescription>
                  )}
                  ...
                  <CardDescription className="text-lg text-amber-200/80">
                   ...
                   <div className="pt-1">
                      <div className="flex justify-between items-baseline mb-1">
                          ...
                          <span className="font-mono text-xs">{character.xp.current.toLocaleString()} / {character.xp.nextLevel.toLocaleString()}</span>
                      </div>
                      ...
                      {character.level >= 20 && (
                          <TooltipProvider>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                <p className="text-xs text-muted-foreground text-center mt-1 cursor-help">
                                    Epic Level Progression available.
                                </p>
                                </TooltipTrigger>
                                <TooltipContent>
                                    <div className="text-sm">
                                        <h4 className="font-bold mb-1">Epic XP Goals:</h4>
                                        <p>Lvl 21: {epicLevelXP[21].next.toLocaleString()} XP</p>
                                        <p>Lvl 22: {epicLevelXP[22].next.toLocaleString()} XP</p>
                                        <p>Lvl 23: {epicLevelXP[23].next.toLocaleString()} XP</p>
                                        <p>Lvl 24: {epicLevelXP[24].next.toLocaleString()} XP</p>
                                        <p>Lvl 25: {epicLevelXP[25].next.toLocaleString()} XP</p>
                                    </div>
                                </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
       ...
       {nobleAssets && (
        <Card className="border-primary/50">
            <CardHeader>
                <CardTitle className="font-headline text-3xl flex items-center gap-2"><Crown /> Royal Assets</CardTitle>
            </CardHeader>
            <CardContent className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                    <h4 className="font-semibold text-lg flex items-center gap-2"><LandPlot /> Land</h4>
                    <p className="text-2xl font-bold">{nobleAssets.land}</p>
                </div>
                <div className="space-y-2">
                    <h4 className="font-semibold text-lg flex items-center gap-2"><UserCog /> Royal Retinue</h4>
                    <ul className="list-disc list-inside text-sm text-muted-foreground">
                        {nobleAssets.retinue.map(adv => <li key={adv.role}>Advisor of {adv.role}</li>)}
                    </ul>
                </div>
                 <div className="space-y-2">
                    <h4 className="font-semibold text-lg flex items-center gap-2"><ShieldCheck /> Military Forces</h4>
                     <div className="text-sm">
                        <p><strong className="text-foreground">Crown Guard:</strong> {nobleAssets.crownGuard} soldiers</p>
                        <p className="font-semibold text-foreground mt-2">King/Queen's Company ({nobleAssets.kingsCompany.total} soldiers):</p>
                        <ul className="list-disc list-inside ml-4 text-muted-foreground">
                            <li>{nobleAssets.kingsCompany.commandingOfficer}</li>
                            <li>{nobleAssets.kingsCompany.executiveOfficer}</li>
                            <li>Engineer Platoon:
                                <ul className="list-['-_'] list-inside ml-4">
                                     <li>{nobleAssets.kingsCompany.engineerPlatoon.leader}</li>
                                     <li>{nobleAssets.kingsCompany.engineerPlatoon.warrantOfficers}</li>
                                     <li>{nobleAssets.kingsCompany.engineerPlatoon.enlisted}</li>
                                </ul>
                            </li>
                        </ul>
                    </div>
           ...
           <Card>
                ...
                <CardContent className="flex flex-wrap justify-center gap-4">
                    {Object.entries(characterStats).map(([stat, value]) => {
                        ...
                        let bonus = 0;
                        if(stat === 'leadership' && character.background === 'noble') {
